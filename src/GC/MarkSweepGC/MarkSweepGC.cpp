...
GC msgc_create(size_t nobjects, size_t size, gc_callback_fn on_mark, gc_callback_fn on_collect, gc_callback_fn on_destroy) 
{
...
void msgc_add(GC gc, size_t nobjects) 
{
     assert(gc);
     size_t i;
     for (i = 0; i < nobjects; i++)
...
void msgc_root(GC gc , void* object) 
{
     assert(gc);
     Root r = NULL;
     if (gc->free_roots) 
...
void msgc_unroot(GC gc , void* object) 
{
     assert(gc);
     Root cur,prev;
...
void msgc_protect(GC gc , void** object) 
{
     assert(gc);
     Root r = NULL;
...
void msgc_expose(GC gc , size_t n) 
{
     assert(gc);
...
void  msgc_mark(gc_callback_fn cont, void* object) 
{
