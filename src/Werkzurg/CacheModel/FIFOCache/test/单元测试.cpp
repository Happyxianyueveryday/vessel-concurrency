...
int main(void)
{
	FIFOCache<Hashable, Hashable> cache(5, noncache_read_callback, noncache_write_callback);
	init_exstore();
	...
	// 3. 移动操作测试
	FIFOCache<Hashable, Hashable> cache2(move(cache));
	FIFOCache<Hashable, Hashable> cache3(1);
	cache3=move(cache2);
	...
	// 5. 顺序写入测试
	{
		FIFOCache<Hashable, Hashable> cache4(cache3);
	...
	// 6. 随机写入测试
	{
		FIFOCache<Hashable, Hashable> cache5(cache3);
		for(int i=0;i<100;i++)
		{
