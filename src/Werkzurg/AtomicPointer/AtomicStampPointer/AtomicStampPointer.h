...
/**
 * load: 读取原子指针
 * note: 读取操作并不修改AtomicStampPointer内部封装的对象指针，因此无需修改版本号 
 */
template<typename T>
AtomicPair<T> AtomicStampPointer<T>::load()
{
...
bool AtomicStampPointer<T>::compare_exchange_strong(T *expected_data, T *new_data, unsigned int expected_version, unsigned int new_version)
{
	...
	return expected_data==current->data&&      
		expected_version==current->version&&    
		((new_data==current->data&&new_version==current->version)||pair.compare_exchange_strong(current, AtomicPair<T>::factory(new_data, new_version)));       // 利用短路求值进行CAS 
}
