 ...
 */
template<typename T>
T* AtomicStampPointer<T>::get_ptr()
{
	return pair.load()->data;
}

/** 
 * compare_exchange_weak: 原子指针CAS操作，对应于std::atomic::compare_exchange_weak 
 * param expected_data: 期望当前的指针值，和标准库的接口不同，当前指针值不会通过expected_value这个参数返回 
 * param new_data: 需要更新的指针值
...
/**
 * compare_exchange_strong: 原子指针CAS操作，对应于std::atomic::compare_exchange_strong 
 */
template<typename T>
