...
void insert_func(ConcurrentStrongHashSet<Hashable> &hashset, int id)
{
	srand(id);
...
void remove_func(ConcurrentStrongHashSet<Hashable> &hashset, int id)
{
	srand(id);
	for(int i=0;i<1000;i++)
	{
...
void contain_func(ConcurrentStrongHashSet<Hashable> &hashset, int id)
{
	srand(id);
	for(int i=0;i<1000;i++)
	{
...
void length_func(ConcurrentStrongHashSet<Hashable> &hashset, int id)
{
	srand(id);
	for(int i=0;i<1000;i++)
	{
...
void print_func(ConcurrentStrongHashSet<Hashable> &hashset, int id)
{
	srand(id);
	for(int i=0;i<5;i++)
	{
...
int main(void)
{
	ConcurrentStrongHashSet<Hashable> hashset;
